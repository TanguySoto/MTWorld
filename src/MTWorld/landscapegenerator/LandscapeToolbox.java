...
public class LandscapeToolbox {
	...
	public static double[][] scaleAndCenter(double[][] landscape, double __scaling,
			double landscapeAltitudeRatio) {
		...
		for (int x = 0; x != dxView; x++)
			for (int y = 0; y != dyView; y++) {
				if (landscape[x][y] < 0) {
					...
					landscape[(x - 1 + dxView) % dxView][(y - 1 + dyView) % dyView] > 0
							|| landscape[x][(y - 1 + dyView) % dyView] > 0
							|| landscape[(x + 1) % dxView][(y - 1 + dyView) % dyView] > 0
							|| landscape[(x - 1 + dxView) % dxView][y] > 0
							|| landscape[(x + 1) % dxView][y] > 0
							|| landscape[(x - 1 + dxView) % dxView][(y + 1 + dyView) % dyView] > 0
							|| landscape[x][(y + 1 + dyView) % dyView] > 0
							|| landscape[(x + 1) % dxView][(y + 1 + dyView) % dyView] > 0) {
						landscape[x][y] = 0.0;
	...
	// Emp�che le sol d'avoir des coordonn�e n�gatives
	public static double[][] avoidUnderZero(double[][] landscape) {
		double[][] myLandscape = landscape.clone();
